...
```go
dailypay.WithServerURL("https://api.example.com")
```
...
```go
dailypay.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
dailypay.WithClient(httpClient)
```
...
```go
dailypay.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
dailypay.WithTimeout(30 * time.Second)
```
...
```go
operations.WithAcceptHeaderOverride(operations.AcceptHeaderEnumApplicationVndApiPlusJson)
```